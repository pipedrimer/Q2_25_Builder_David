...
[mintA, mintB].map((b)=> getAssociatedTokenAddressSync(b.publicKey, a.publicKey, false, tokenProgram)) ).flat();
...
tx.instructions= [
  ...
  ].flatMap((x)=> [createInitializeMint2Instruction(x.mint, 6, x.authority, null, tokenProgram),
...
it("Make", async ()=> {
  Object.entries(accounts).map(([key, value])=> console.log(`${key} : ${value.toString()}`))
